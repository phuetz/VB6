...
export const useDragDrop = (options: DragDropOptions = {}) => {
  ...
  const makeDraggable = useCallback((element: HTMLElement, item: any) => {
  ...
  const makeDroppable = useCallback((element: HTMLElement, target: any) => {
...
export const useVB6ControlDragDrop = (onCreateControl: (type: string, x: number, y: number) => void) => {
  ...
  const handleCanvasDrop = useCallback((e: DragEvent, canvas: HTMLElement) => {
    e.preventDefault();
    
    const controlType = e.dataTransfer?.getData('application/vb6-control') || 
...
export const useControlResize = (onResize: (id: string, newBounds: { x: number; y: number; width: number; height: number }) => void) => {
