...
export const CommandWindow: React.FC<CommandWindowProps> = ({
        ...
        parameters: [
          {
            ...
            type: 'file',
            required: true,
            description: 'Path to the VBP project file',
          },
        ],
        ...
        parameters: [
          {
            ...
            type: 'string',
            required: true,
            description: 'Name of the window to show',
            validValues: [
              'Properties',
              'Toolbox',
              'ProjectExplorer',
        ...
        parameters: [
          {
            ...
            type: 'string',
            required: true,
            description: 'Name of the window to hide',
          },
        ],
        ...
        parameters: [
          {
            ...
            type: 'string',
            required: false,
            description: 'Options category to open',
          },
        ],
  ...
  const addToHistory = useCallback(
    (command: string, status: CommandStatus, result?: string, error?: string) => {
    ...
    async (commandText: string) => {
      ...
      try {
        ...
        setCommandHistory(prev => ({
          ...prev,
          commands: [commandText, ...prev.commands.filter(cmd => cmd !== commandText)].slice(
            0,
            prev.maxItems
          ),
          currentIndex: -1,
    ...
    switch (command.name) {
      ...
      case 'Help':
        if (args.length === 0) {
          ...
          const helpCommand = availableCommands.find(
            c => c.name.toLowerCase() === args[0].toLowerCase()
          );
          if (helpCommand) {
    ...
    <div className="flex flex-col h-full bg-white border border-gray-300">
      ...
      <div className="flex items-center justify-between p-2 bg-gray-100 border-b border-gray-300">
        <div className="flex items-center gap-2">
          ...
          {isExecuting && <div className="text-xs text-blue-600 animate-pulse">Executing...</div>}
        </div>
      ...
      {/* Output Area */}
      <div ref={outputRef} className="flex-1 overflow-y-auto p-2 font-mono text-sm bg-white">
        {executionHistory.map(execution => (
          <div key={execution.id} className="mb-2">
            ...
            {execution.result && (
              <div className={`ml-6 ${getStatusColor(execution.status)}`}>{execution.result}</div>
            )}
            {execution.error && <div className="ml-6 text-red-600">Error: {execution.error}</div>}
          </div>
        ))}

        {isExecuting && <div className="text-blue-600 animate-pulse">Executing command...</div>}
      </div>
                ...
                {selectedCommand.examples.length > 0 && (
                  <div className="mb-4">
                    ...
                    {selectedCommand.examples.map((example, index) => (
                      <code key={index} className="bg-gray-100 p-2 rounded block mb-1 text-sm">
                        {example}
                      </code>
                    ))}
                  </div>
              ...
              <div>
                ...
                {availableCommands
                  .filter(
                    cmd =>
                      cmd.isAvailable &&
                      (filterCategory === 'All' || cmd.category === filterCategory)
                  )
                  .map(command => (
                    <div
                      key={command.name}
