...
export const MenuEditor: React.FC<MenuEditorProps> = ({
  ...
  const createMenuResource = useCallback(
    (name: string): MenuResource => ({
      id: generateId(),
      name,
      items: [],
  ...
  const addMenuItem = useCallback(
    (parent?: MenuItem, type: MenuItemType = MenuItemType.MenuItem) => {
  ...
  const updateMenuItem = useCallback(
    (item: MenuItem, updates: Partial<MenuItem>) => {
      if (!selectedMenu) return;
  ...
  const moveMenuItem = useCallback(
    (sourceItem: MenuItem, targetItem: MenuItem, position: 'before' | 'after' | 'child') => {
  ...
  const handleDrop = useCallback(
    (e: React.DragEvent, targetItem: MenuItem, position: 'before' | 'after' | 'child') => {
      e.preventDefault();
      ...
      return items.map(item => (
        <div key={item.id} className="select-none">
          ...
          >
            ...
            {item.accelerator && (
              <span className="text-xs text-gray-500">{getAcceleratorText(item.accelerator)}</span>
            )}
        ...
        <div className="space-y-4">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                >
                  ...
                  {Object.values(AcceleratorKey)
                    .filter(k => k !== AcceleratorKey.None)
                    .map(key => (
          ...
          <div>
            ...
            <input
              ...
              onChange={e =>
                updateMenuItem(selectedItem, {
                  helpId: e.target.value ? Number(e.target.value) : undefined,
                })
              }
              className="w-full px-3 py-2 border border-gray-300 rounded"
          ...
          <div className="bg-white rounded-lg p-6 w-96">
            ...
            <div className="space-y-3">
              <button
                onClick={() =>
                  addMenuItem(
                    selectedItem?.type === MenuItemType.Popup ? selectedItem : undefined,
              ...
              >
                ...
                <div className="text-sm text-gray-600">
                  Horizontal line to separate menu sections
                </div>
              </button>
            </div>
