...
import { X, Plus, Trash2, ChevronLeft, ChevronRight, Check, FileCode, Layout, Settings, Code, Eye, Copy, Download, Folder, Monitor, Database, Globe, Wrench, Shield, Palette, Target, Grid, List, Menu, Toolbar, Users, Info, AlertTriangle, Zap } from 'lucide-react';
...
export const ApplicationWizard: React.FC<ApplicationWizardProps> = ({
  ...
  const [wizardData, setWizardData] = useState<ApplicationConfiguration>(() => configuration || {
    id: `app-${Date.now()}`,
    name: 'MyApplication',
  ...
  useEffect(() => {
    ...
    const updatedFeatures = availableFeatures.map(feature => {
      ...
      if (wizardData.template === ApplicationTemplate.DATABASE_FORM || 
      ...
      if (wizardData.template === ApplicationTemplate.MDI) {
        if (feature.id === 'menu-bar' || feature.id === 'toolbar' || feature.id === 'status-bar') enabled = true;
      }
  ...
  const generateProjectFiles = (): Record<string, string> => {
    ...
    wizardData.references.filter(ref => ref.selected && !ref.required).forEach(ref => {
      if (ref.guid) {
    ...
    wizardData.references.filter(ref => ref.selected && ref.filename.endsWith('.OCX')).forEach(ref => {
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-sm text-gray-600">
                    ...
                    {template === ApplicationTemplate.MDI && 'Multiple document interface with child windows'}
                    {template === ApplicationTemplate.DIALOG && 'Simple dialog-based application'}
                    {template === ApplicationTemplate.DATABASE_FORM && 'Form-based database application'}
                    {template === ApplicationTemplate.DATABASE_EXPLORER && 'Database browsing and editing tool'}
                    {template === ApplicationTemplate.WEB_BROWSER && 'Web browser with navigation controls'}
                    {template === ApplicationTemplate.TEXT_EDITOR && 'Basic text editing application'}
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <select
          ...
          onChange={(e) => setWizardData({ ...wizardData, interfaceStyle: e.target.value as InterfaceStyle })}
          className="w-full px-3 py-2 border rounded focus:ring-2 focus:ring-blue-500"
        >
      ...
      <div>
        ...
        <div className="space-y-1 max-h-96 overflow-y-auto">
          {['Interface', 'Functionality', 'Data', 'Internet', 'Multimedia', 'Documentation', 'Internationalization', 'Configuration'].map(category => (
            <div key={category}>
              ...
              {availableFeatures.filter(f => f.category === category).map(feature => (
                <div key={feature.id} className="flex items-start gap-3 p-2 border rounded hover:bg-gray-50">
                  <input
                    type="checkbox"
                    ...
                    {feature.dependsOn && (
                      <div className="text-xs text-orange-600 mt-1">
                        Requires: {feature.dependsOn.map(dep => 
                          availableFeatures.find(f => f.id === dep)?.name || dep
                        ).join(', ')}
                      </div>
                    )}
                  </div>
          ...
          <div className="space-y-3">
            <label className="flex items-center gap-2">
              <input
                ...
                onChange={(e) => setWizardData({
                  ...wizardData,
                  codeGeneration: { ...wizardData.codeGeneration, generateComments: e.target.checked }
                })}
              />
            ...
            <label className="flex items-center gap-2">
              <input
                ...
                onChange={(e) => setWizardData({
                  ...wizardData,
                  codeGeneration: { ...wizardData.codeGeneration, useErrorHandling: e.target.checked }
                })}
              />
            ...
            <label className="flex items-center gap-2">
              <input
                ...
                onChange={(e) => setWizardData({
                  ...wizardData,
                  codeGeneration: { ...wizardData.codeGeneration, generateLogging: e.target.checked }
                })}
              />
            ...
            <label className="flex items-center gap-2">
              <input
                ...
                onChange={(e) => setWizardData({
                  ...wizardData,
                  codeGeneration: { ...wizardData.codeGeneration, includeVersionInfo: e.target.checked }
                })}
              />
            ...
            <label className="flex items-center gap-2">
              <input
                ...
                onChange={(e) => setWizardData({
                  ...wizardData,
                  codeGeneration: { ...wizardData.codeGeneration, generateResourceFile: e.target.checked }
                })}
              />
        ...
        <div>
          ...
          <div className="space-y-3">
            <label className="flex items-center gap-2">
              <input
                ...
                onChange={(e) => setWizardData({
                  ...wizardData,
                  deploymentOptions: { ...wizardData.deploymentOptions, createSetup: e.target.checked }
                })}
              />
            ...
            <label className="flex items-center gap-2">
              <input
                ...
                onChange={(e) => setWizardData({
                  ...wizardData,
                  deploymentOptions: { ...wizardData.deploymentOptions, includeRuntime: e.target.checked }
                })}
              />
            ...
            <label className="flex items-center gap-2">
              <input
                ...
                onChange={(e) => setWizardData({
                  ...wizardData,
                  deploymentOptions: { ...wizardData.deploymentOptions, createShortcuts: e.target.checked }
                })}
              />
            ...
            <label className="flex items-center gap-2">
              <input
                ...
                onChange={(e) => setWizardData({
                  ...wizardData,
                  deploymentOptions: { ...wizardData.deploymentOptions, registerComponents: e.target.checked }
                })}
              />
          ...
          <div className="mt-4">
            ...
            <input
              ...
              onChange={(e) => setWizardData({
                ...wizardData,
                deploymentOptions: { ...wizardData.deploymentOptions, targetFolder: e.target.value }
              })}
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="flex gap-2 mb-4">
          <button
            onClick={() => {
              const zip = Object.entries(projectFiles).map(([filename, content]) => 
                `// ${filename}\n${content}\n\n`
              ).join('');
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="bg-green-50 border border-green-200 rounded p-4">
            ...
            <div className="text-sm text-green-700 space-y-1">
              <div>Type: <strong>{wizardData.projectType}</strong></div>
              <div>Template: <strong>{wizardData.template}</strong></div>
              <div>Features: <strong>{wizardData.features.filter(f => f.enabled).length}</strong></div>
              <div>References: <strong>{wizardData.references.filter(r => r.selected).length}</strong></div>
              <div>Files: <strong>{Object.keys(projectFiles).length}</strong></div>
            </div>
          </div>
