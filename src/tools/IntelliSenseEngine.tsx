...
export class VB6LanguageDatabase {
  ...
  private initializeDatabase(): void {
    ...
    this.addFunction('CInt', 'Converts an expression to an Integer', 'Integer', [
      {
        label: 'expression',
        type: 'Variant',
        documentation: 'Any valid string or numeric expression',
      },
    ]);
      ...
      [
        {
          label: 'interval',
          type: 'String',
          documentation: 'String expression that is the interval you want to add',
        },
    ...
    this.addFunction('DateDiff', 'Returns the number of time intervals between two dates', 'Long', [
      {
        label: 'interval',
        type: 'String',
        documentation: 'String expression that is the interval you want to use',
      },
    ...
    this.addFunction('DatePart', 'Returns the specified part of a given date', 'Integer', [
      {
        label: 'interval',
        type: 'String',
        documentation: 'String expression that is the interval you want to return',
      },
      ...
      [
        {
          label: 'pathname',
          type: 'String',
          documentation: 'String expression that specifies a file name',
          optional: true,
        },
        {
          label: 'attributes',
          type: 'VbFileAttribute',
          documentation: 'Constant or numeric expression',
          optional: true,
        },
      ]
    );
    ...
    this.addFunction(
      ...
      [
        { label: 'expression', type: 'Variant', documentation: 'Any valid expression' },
        {
          label: 'format',
          type: 'String',
          documentation: 'A valid named or user-defined format expression',
          optional: true,
        },
      ]
    );
      ...
      [
        {
          label: 'start',
          type: 'Long',
          documentation: 'Starting position for each search',
          optional: true,
        },
        ...
        { label: 'string2', type: 'String', documentation: 'String being searched for' },
        {
          ...
          documentation: 'Comparison method',
          optional: true,
        },
      ]
    );
      ...
      [
        {
          label: 'string',
          type: 'String',
          documentation: 'String expression from which the leftmost characters are returned',
        },
    ...
    this.addFunction('Mid', 'Returns a specified number of characters from a string', 'String', [
      {
        label: 'string',
        type: 'String',
        documentation: 'String expression from which characters are returned',
      },
    ...
    this.addFunction('MsgBox', 'Displays a message in a dialog box', 'VbMsgBoxResult', [
      {
        label: 'prompt',
        type: 'String',
        documentation: 'String expression displayed as the message',
      },
      {
        label: 'buttons',
        type: 'VbMsgBoxStyle',
        documentation: 'Sum of values that specify buttons and icon',
        optional: true,
      },
      {
        label: 'title',
        type: 'String',
        documentation: 'String expression displayed in title bar',
        optional: true,
      },
      {
        label: 'helpfile',
        type: 'String',
        documentation: 'String expression identifying Help file',
        optional: true,
      },
    ...
    this.addFunction(
      'Now',
      'Returns the current date and time according to the setting of your system',
      'Variant',
      []
    );
      ...
      [
        {
          label: 'expression',
          type: 'String',
          documentation: 'String expression containing substring to replace',
        },
        ...
        { label: 'start', type: 'Long', documentation: 'Position to begin search', optional: true },
        {
          label: 'count',
          type: 'Long',
          documentation: 'Number of substitutions to perform',
          optional: true,
        },
        {
          label: 'compare',
      ...
      [
        {
          label: 'string',
          type: 'String',
          documentation: 'String expression from which the rightmost characters are returned',
        },
        { label: 'length', type: 'Long', documentation: 'Number of characters to return' },
      ]
    );
      ...
      [
        {
          label: 'expression',
          type: 'String',
          documentation: 'String expression containing substrings and delimiters',
        },
        {
          label: 'delimiter',
          type: 'String',
          documentation: 'String character used to identify substring limits',
          optional: true,
        },
        {
          label: 'limit',
          type: 'Long',
          documentation: 'Number of substrings to be returned',
          optional: true,
        },
        {
          label: 'compare',
    ...
    this.addFunction(
      ...
      [
        { label: 'number', type: 'Long', documentation: 'Length of the returned string' },
        {
          label: 'character',
          type: 'Variant',
          documentation: 'Character code or string expression',
        },
      ]
    );
    ...
    this.addObject('Screen', 'Screen object', [
      ...
      { name: 'Width', type: 'Single', documentation: 'Width of the screen in twips' },
      {
        name: 'TwipsPerPixelX',
        type: 'Single',
        documentation: 'Number of twips per pixel horizontally',
      },
      {
        name: 'TwipsPerPixelY',
        type: 'Single',
        documentation: 'Number of twips per pixel vertically',
      },
    ]);
  ...
  private addObject(
    name: string,
    documentation: string,
    members: Array<{ name: string; type: string; documentation: string }>
  ): void {
...
export class IntelliSenseEngine extends EventEmitter {
  ...
  private parseLine(line: string, lineIndex: number): void {
    ...
    const funcMatch = trimmedLine.match(
      /^\s*(Private\s+|Public\s+)?Function\s+(\w+)\s*\([^)]*\)\s+As\s+(\w+)/i
    );
    if (funcMatch) {
  ...
  private isKnownIdentifier(identifier: string): boolean {
    ...
    return (
      this._database.findCompletion(lower) !== undefined ||
  ...
  getSignatureHelp(position: Position, context: string): SignatureHelp | null {
    ...
    const completion =
      this._database.findCompletion(funcName) || this._userSymbols.get(funcName.toLowerCase());
  ...
  getHover(position: Position, word: string): Hover | null {
    const completion =
      this._database.findCompletion(word) || this._userSymbols.get(word.toLowerCase());

    if (!completion) return null;

    const contents = [completion.detail || completion.label, completion.documentation || ''].filter(
      Boolean
    );
  ...
  formatDocument(content: string): string {
    ...
    lines.forEach(line => {
      ...
      if (
        /^\s*(If\b.*Then\s*$|Function\b|Sub\b|With\b|Select\s+Case\b|For\b|Do\b|While\b)/i.test(
          trimmed
        )
      ) {
        indentLevel++;
      ...
      >
        {completions.map((completion, index) => (
          ...
          >
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontWeight: 'bold', color: '#333' }}>{completion.label}</div>
              {completion.detail && (
