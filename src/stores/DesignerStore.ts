...
interface DesignerActions {
  ...
  // Opérations de drag & drop
  startDrag: (
    type: 'create' | 'move' | 'resize' | 'select',
    ...
    subscribeWithSelector(
      immer((set, get) => ({
        ...
        createControl: (
          ...
          const position = state.canvas.snapToGrid ? get().snapToGrid(x, y) : { x, y };
        ...
        deleteControls: (controlIds: number[]) =>
          set(state => {
            ...
            state.selection.selectedControlIds = state.selection.selectedControlIds.filter(
        ...
        selectMultipleControls: (controlIds: number[]) =>
          set(state => {
            const validIds = controlIds.filter(id => state.controls.some(c => c.id === id));
          ...
          set(state => {
            ...
            if (dragType === 'create' && state.drag.previewControl) {
              ...
              const finalPos = state.canvas.snapToGrid
                ? get().snapToGrid(endPos.x, endPos.y)
                : endPos;

              get().createControl(state.drag.previewControl.type, finalPos.x, finalPos.y);
            }
        ...
        pasteControls: () => {
          ...
          set(draft => {
            ...
            if (draft.clipboard.operation === 'cut') {
              const originalIds = draft.clipboard.controls.filter(c => c.id !== -1).map(c => c.id);
        ...
        alignLeft: () => {
          ...
          const selectedControls = controls.filter(c =>
            selection.selectedControlIds.includes(c.id)
          );
