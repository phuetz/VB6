...
export class VB6Lexer {
  ...
  private readOperatorOrPunctuation(line: number, column: number): VB6Token {
    ...
    switch (char) {
      ...
      case '\\':
        return { type: TokenType.DIVIDE, value, line, column }; // Integer division
      case '^':
...
export class VB6Parser {
  ...
  private isDeclaration(): boolean {
    return (
      this.check(TokenType.KEYWORD_DIM) ||
  ...
  private isProcedure(): boolean {
    return (
      this.check(TokenType.KEYWORD_SUB) ||
  ...
  private consume(type: TokenType): VB6Token {
    ...
    throw new Error(
      `Expected ${type} but got ${this.current().type} at line ${this.current().line}`
    );
  }
