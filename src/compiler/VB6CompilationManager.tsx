...
export const VB6CompilationManager: React.FC<VB6CompilationManagerProps> = ({
  ...
  const processConditionalCompilation = useCallback(
    (code: string, constants: CompilationConstant[]): string => {
      ...
      lines.forEach((line, index) => {
        ...
        if (trimmedLine.startsWith('#If ')) {
          ...
          if (directiveStack.length > 0) {
            ...
            const parentActive =
              directiveStack.length > 1 ? directiveStack[directiveStack.length - 2].active : true;
            const active =
              parentActive &&
              !directiveStack[directiveStack.length - 1].active &&
          ...
          if (directiveStack.length > 0) {
            directiveStack.pop();
            currentActive =
              directiveStack.length > 0 ? directiveStack[directiveStack.length - 1].active : true;
          }
    ...
    try {
      ...
      const processedFiles = projectFiles.map((file, index) => {
        ...
        if (file.type === 'vb' || file.type === 'bas' || file.type === 'cls') {
          processedContent = processConditionalCompilation(
            file.content,
            buildSettings.compilationConstants
          );
        }
      ...
      const outputExtension =
        buildSettings.compilationType === CompilationType.ActiveXDLL ? '.dll' : '.exe';
  ...
  const previewConditionalCompilation = useCallback(() => {
    if (selectedFile && projectFiles.length > 0) {
      ...
      if (file) {
        const processed = processConditionalCompilation(
          file.content,
          buildSettings.compilationConstants
        );
          ...
          <div className="flex items-center gap-4">
            ...
            <select
              ...
              onChange={e =>
                setBuildSettings(prev => ({
                  ...prev,
                  configuration: e.target.value as BuildConfiguration,
                }))
              }
        ...
        <div className="p-4 bg-blue-50 border-b border-blue-200">
          ...
          {buildProgress.currentFile && (
            <div className="text-xs text-blue-600 mt-1">
              Processing: {buildProgress.currentFile} ({buildProgress.processedFiles + 1}/
              {buildProgress.totalFiles})
            </div>
          )}
        </div>
      )}
      ...
      <div className="flex flex-1 overflow-hidden">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  onChange={e =>
                    setBuildSettings(prev => ({ ...prev, outputName: e.target.value }))
                  }
              ...
              <div>
                ...
                <select
                  ...
                  onChange={e =>
                    setBuildSettings(prev => ({
                      ...prev,
                      targetPlatform: e.target.value as TargetPlatform,
                    }))
                  }
                  className="w-full px-2 py-1 border border-gray-300 rounded text-sm"
                >
              ...
              <div className="space-y-2">
                <label className="flex items-center gap-2 text-sm">
                  <input
                    ...
                    onChange={e =>
                      setBuildSettings(prev => ({ ...prev, generateDebugInfo: e.target.checked }))
                    }
                  />
                  Generate Debug Info
                </label>
                ...
                <label className="flex items-center gap-2 text-sm">
                  <input
                    ...
                    onChange={e =>
                      setBuildSettings(prev => ({ ...prev, removeUnusedCode: e.target.checked }))
                    }
                  />
                  Remove Unused Code
                </label>
                ...
                <label className="flex items-center gap-2 text-sm">
                  <input
                    ...
                    onChange={e =>
                      setBuildSettings(prev => ({ ...prev, compileOnDemand: e.target.checked }))
                    }
                  />
                  Compile On Demand
                </label>
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={e =>
                        setBuildSettings(prev => ({
                          ...prev,
                          threadingModel: e.target.value as 'Apartment' | 'Free' | 'Both',
                        }))
                      }
                      className="w-full px-2 py-1 border border-gray-300 rounded text-sm"
                    >
                  ...
                  <div className="space-y-2">
                    <label className="flex items-center gap-2 text-sm">
                      <input
                        ...
                        onChange={e =>
                          setBuildSettings(prev => ({ ...prev, makeCompatible: e.target.checked }))
                        }
                      />
                    ...
                    <label className="flex items-center gap-2 text-sm">
                      <input
                        ...
                        onChange={e =>
                          setBuildSettings(prev => ({
                            ...prev,
                            unattendedExecution: e.target.checked,
                          }))
                        }
                      />
                    ...
                    <label className="flex items-center gap-2 text-sm">
                      <input
                        ...
                        onChange={e =>
                          setBuildSettings(prev => ({ ...prev, threadPerObject: e.target.checked }))
                        }
                      />
                      Thread Per Object
                    </label>
                ...
                {buildSettings.compilationConstants.map((constant, index) => (
                  <div key={index} className="p-2 border border-gray-300 rounded">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <button
                        onClick={() => {
                          const updated = buildSettings.compilationConstants.filter(
                            (_, i) => i !== index
                          );
                          setBuildSettings(prev => ({ ...prev, compilationConstants: updated }));
                        }}
        ...
        {compilationResult && (
          <div className="w-96 bg-white border-l border-gray-200 overflow-y-auto">
            <div className="p-4">
              <h3
                className={`font-medium mb-4 ${compilationResult.success ? 'text-green-700' : 'text-red-700'}`}
              >
                ...
                {compilationResult.success && (
                  <>
                    ...
                    <div>
                      ...
                      {(compilationResult.outputSize! / 1024).toFixed(1)} KB
                    </div>
                  </>
                )}
