...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const currentData =
      recordset.position >= 0 && recordset.position < recordset.rows.length
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (filter === 0) {
      ...
      recordset.filteredRows = recordset.rows.filter(row => evaluateFilterCriteria(row, criteria));
    }
