 ...
 */
export function enhanceAllControls(controls: {
  [controlId: string]: { type: string; properties: ControlPropertyState };
}): {
...
export function generateCompatibilityReport(): {
  ...
  const averageCompatibility =
    reports.reduce((sum, r) => sum + r.compatibility, 0) / reports.length;
...
export function compareControlProperties(
  ...
  const modifiedProperties = props1.filter(
    p => props2.includes(p) && properties1[p] !== properties2[p]
  );
