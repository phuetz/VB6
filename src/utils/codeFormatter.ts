...
const VB6_KEYWORDS = [
  'Dim', 'As', 'Set', 'Let', 'Sub', 'Function', 'End', 'If', 'Then', 'Else', 'ElseIf',
  'For', 'To', 'Next', 'While', 'Wend', 'Do', 'Loop', 'Until', 'Select', 'Case',
  'Private', 'Public', 'Friend', 'Static', 'Const', 'Type', 'Enum', 'Property',
  'Get', 'Let', 'Set', 'WithEvents', 'New', 'Nothing', 'True', 'False', 'And',
  'Or', 'Not', 'Xor', 'Eqv', 'Imp', 'Is', 'Mod', 'Call', 'Option', 'Explicit',
  'On', 'Error', 'Resume', 'GoTo', 'GoSub', 'Return', 'Exit', 'Declare', 'ReDim',
  'Preserve', 'Integer', 'Long', 'Single', 'Double', 'String', 'Currency', 'Date',
  'Boolean', 'Byte', 'Object', 'Variant', 'With', 'In', 'Each', 'Print', 'Debug'
];
...
function isEndStatement(line: string): boolean {
  return /^\s*(End\s+(Sub|Function|If|Property|With|Type|Select|Enum)|Next(\s+\w+)?|Loop|Wend)\b/i.test(line);
}
...
function isStartStatement(line: string): boolean {
  ...
  if (/^\s*If\s+.*\s+Then\s*(?!.*\s+End\s+If)/i.test(line) && !/^\s*If\s+.*\s+Then\s*.*\s+Else\s*/i.test(line)) {
    return true;
  }
  
  if (/^\s*(For\s+|Do\s+While\s+|Do\s+Until\s+|Do\s*$|With\s+|Select\s+Case\s+|Type\s+|Enum\s+)\b/i.test(line)) {
    return true;
  }
...
function addSpaceAfterKeywords(lines: string[], keywords: string[]): string[] {
  ...
  const keywordsRequiringSpace = keywords.filter(k => 
    !['As', 'To', 'Is', 'New', 'Nothing', 'Then', 'And', 'Or', 'Not', 'Xor'].includes(k)
  );
