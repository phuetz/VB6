...
export function FormatCurrency(
  ...
  if (groupDigits === VB6FormatConstants.vbTrue || 
...
export function FormatDateTime(
  ...
  switch (namedFormat) {
    ...
    default: {
      ...
      const hasTimeComponent = dateObj.getHours() !== 0 || dateObj.getMinutes() !== 0 || dateObj.getSeconds() !== 0;
...
export function Format(
  ...
  // Handle date formats
  if (expression instanceof Date || (typeof expression === 'string' && !isNaN(Date.parse(expression)))) {
    return formatDate(expression instanceof Date ? expression : new Date(expression), format);
  }
  
  // Handle number formats
  if (typeof expression === 'number' || (typeof expression === 'string' && !isNaN(parseFloat(expression)))) {
 ...
 */
export function WeekdayName(weekday: number, abbreviate: boolean = false, firstDayOfWeek: number = 1): string {
  const weekdays = [
