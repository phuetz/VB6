...
export class VB6SystemFunctions {
    ...
    if (typeof process !== 'undefined' && process.env) {
      if (typeof expression === 'string') {
        ...
        if (SAFE_ENV_VARS.includes(expression)) {
          ...
          console.warn(
            `Access to environment variable '${expression}' blocked for security reasons`
          );
  ...
  static CreateObject(className: string, serverName?: string): any {
    ...
    const mockObjects: { [key: string]: any } = {
      ...
      'Scripting.Dictionary': {
        ...
        Exists: function (key: any) {
          return this.items.has(key);
        },
        Item: function (key: any) {
  ...
  private static parseSendKeysString(
    keys: string
  ): Array<string | { key: string; code: string; ctrl?: boolean; shift?: boolean; alt?: boolean }> {
