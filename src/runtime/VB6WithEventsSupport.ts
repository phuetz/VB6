...
export class VB6EventManager {
  ...
  declareEvent(
    ...
    console.log(`[VB6 Events] Declared event ${className}.${eventName}`);
  }
  ...
  declareWithEvents(
    ...
    console.log(`[VB6 Events] Declared WithEvents ${propertyName} As ${objectType}`);
  }
  ...
  setWithEventsObject(
    ...
    console.log(`[VB6 Events] Set WithEvents object ${propertyName}`);
  }
  ...
  raiseEvent(
    ...
    try {
      ...
      if (listeners && listeners.size > 0) {
        console.log(`[VB6 Events] Raising ${eventName} with ${listeners.size} listeners`);
  ...
  addEventListener(
    ...
    console.log(`[VB6 Events] Added listener for ${eventName}`);
  }
  ...
  registerWithEventsHandler(
    ...
    console.log(`[VB6 Events] Registered WithEvents handler ${handlerKey}`);
  }
  ...
  private raiseWithEventsHandlers(
    source: any,
    eventName: string,
    args: any[]
  ): void {
    ...
    console.log(`[VB6 Events] Checking WithEvents handlers for ${eventName}`);
  }
...
export class VB6FormWithEvents {
  ...
  EventSource_StatusChanged(status: string, code: number): void {
    console.log(`Status changed to: ${status} (code: ${code})`);
  }
...
// Declare WithEvents
export function DeclareWithEvents(
  owner: any,
  propertyName: string,
  objectType: string
): void {
