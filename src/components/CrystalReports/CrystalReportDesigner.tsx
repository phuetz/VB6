...
import {
  ...
  ReportParameter,
  ReportChart,
} from '../../server/src/services/CrystalReportsService';
...
export const CrystalReportDesigner: React.FC<CrystalReportDesignerProps> = ({
  ...
  const handleDrop = useCallback(
    (e: React.DragEvent, sectionType: string) => {
      e.preventDefault();
            ...
            {section.objects.map(obj => (
              <motion.div
                ...
                style={{
                  ...
                  display: 'flex',
                  alignItems:
                    obj.formatting.verticalAlignment === 'top'
                      ? 'flex-start'
                      : obj.formatting.verticalAlignment === 'bottom'
                        ? 'flex-end'
                        : 'center',
                  justifyContent:
                    obj.formatting.alignment === 'left'
                      ? 'flex-start'
                      : obj.formatting.alignment === 'right'
                        ? 'flex-end'
                        : 'center',
                  padding: '2px',
                  overflow: 'hidden',
              ...
              >
                ...
                {obj.type === 'line' && <div className="w-full h-px bg-black" />}
                {obj.type === 'box' && <div className="w-full h-full border border-black" />}
                {obj.type === 'chart' && (
  ...
  const renderPropertyPanel = () => {
    if (!selectedObject) {
      return (
        <div className="p-4 text-gray-500">Sélectionnez un objet pour voir ses propriétés</div>
      );
    }
        ...
        <div className="border-t pt-4">
          ...
          <div className="space-y-2">
            ...
            <select
              ...
              onChange={e =>
                updateObjectProperty('formatting', {
  ...
  return (
    <div className={`flex h-screen ${theme === 'dark' ? 'bg-gray-900' : 'bg-gray-50'}`}>
      ...
      <div className="flex-1 flex flex-col">
        {/* Barre d'outils */}
        <div
          className={`flex items-center justify-between p-4 ${theme === 'dark' ? 'bg-gray-800' : 'bg-white'} border-b border-gray-300`}
        >
        ...
        {/* Onglets */}
        <div
          className={`flex border-b ${theme === 'dark' ? 'border-gray-600' : 'border-gray-300'}`}
        >
              ...
              {showRuler && (
                <div className="absolute top-0 left-0 w-full h-6 bg-gray-200 border-b border-gray-300 z-10">
                  ...
                  <div className="relative h-full">
                    {Array.from(
                      { length: Math.ceil(currentReport.pageSettings.width / 10) },
                      (_, i) => (
                        <div
                          key={i}
