...
const PropertyGrid: React.FC<PropertyGridProps> = ({
  ...
  const [expandedCategories, setExpandedCategories] = useState<Set<string>>(new Set(['Appearance', 'Behavior', 'Layout']));
  ...
  const getPropertyIcon = useCallback((propDef: PropertyDefinition) => {
    ...
    switch (propDef.type) {
      case 'string': return <span className="text-blue-600 font-mono text-xs">Ab</span>;
      case 'number': return <span className="text-green-600 font-mono text-xs">123</span>;
      case 'boolean': return <span className="text-purple-600 font-mono text-xs">T/F</span>;
      case 'color': return <div className="w-3 h-3 border border-gray-400 bg-gradient-to-r from-red-500 to-blue-500 rounded-sm" />;
      case 'font': return <span className="text-gray-600 font-serif text-xs italic">Aa</span>;
      case 'enum': return <span className="text-orange-600 text-xs">▼</span>;
      case 'picture': return <div className="w-3 h-3 border border-gray-400 bg-gray-200 flex items-center justify-center text-xs">🖼</div>;
      case 'code': return <span className="text-indigo-600 font-mono text-xs">{}</span>;
      case 'datasource': return <span className="text-teal-600 font-mono text-xs">DB</span>;
      case 'menu': return <span className="text-gray-600 text-xs">☰</span>;
      case 'icon': return <span className="text-yellow-600 text-xs">⚡</span>;
      case 'cursor': return <span className="text-gray-600 text-xs">↖</span>;
  ...
  const filteredAndSortedProperties = useMemo(() => {
    const filtered = properties.filter(prop => {
      // Text filter
      if (filterConfig.text && !prop.name.toLowerCase().includes(filterConfig.text.toLowerCase()) &&
  ...
  const handlePropertyChange = useCallback((propertyName: string, newValue: any) => {
    ...
    const finalValue = validationResult.correctedValue !== undefined ? validationResult.correctedValue : newValue;
  ...
  const renderPropertyEditor = useCallback((propDef: PropertyDefinition, value: any) => {
    ...
    switch (propDef.type) {
      case 'color':
        return <div className={editorClassName}><ColorEditor {...editorProps} /></div>;
      
      case 'font':
        return <div className={editorClassName}><FontEditor {...editorProps} /></div>;
      
      case 'picture':
        return <div className={editorClassName}><PictureEditor {...editorProps} /></div>;
      
      case 'enum':
        return <div className={editorClassName}><EnumEditor {...editorProps} enumValues={propDef.enumValues || []} /></div>;
      
      case 'boolean':
        return <div className={editorClassName}><BooleanEditor {...editorProps} /></div>;
      
      case 'number':
        return <div className={editorClassName}><NumberEditor {...editorProps} min={propDef.validation?.min} max={propDef.validation?.max} /></div>;
      
      case 'code':
        return <div className={editorClassName}><CodeEditor {...editorProps} /></div>;
      
      case 'datasource':
        return <div className={editorClassName}><DataSourceEditor {...editorProps} /></div>;
      
      case 'menu':
        return <div className={editorClassName}><MenuEditor {...editorProps} /></div>;
      
      case 'icon':
        return <div className={editorClassName}><IconEditor {...editorProps} type="icon" /></div>;
      
      case 'cursor':
        return <div className={editorClassName}><IconEditor {...editorProps} type="cursor" /></div>;
      
      case 'string':
      default:
        return <div className={editorClassName}><StringEditor {...editorProps} multiline={propDef.name === 'ToolTipText' || propDef.name === 'Tag'} /></div>;
    }
  }, [editingProperty, validationResults, handlePropertyChange, readOnly]);

  // Render validation indicator
  const renderValidationIndicator = useCallback((propertyName: string) => {
      ...
      <div className="vb6-properties-toolbar">
        <div className="flex items-center gap-1">
          <button 
            ...
            title="Sort by Name"
          >
            {sortConfig.field === 'name' && sortConfig.direction === 'desc' ? <SortDesc size={12} /> : <SortAsc size={12} />}
          </button>
          ...
          <div className="flex gap-4 mt-2 text-xs">
            ...
            <label className="flex items-center">
              <input
                ...
                onChange={(e) => setFilterConfig(prev => ({ ...prev, showReadOnly: e.target.checked }))}
                className="mr-1"
              />
              Show Read-Only
            </label>
            ...
            <label className="flex items-center">
              <input
                ...
                onChange={(e) => setFilterConfig(prev => ({ ...prev, showAdvanced: e.target.checked }))}
                className="mr-1"
              />
              Show Advanced
            </label>
      ...
      <div className="vb6-properties-list">
        {propertyGroups.map(group => (
          <div key={group.category}>
            ...
            {/* Properties */}
            {(!showCategories || group.expanded) && group.properties.map(propDef => {
              ...
              return (
                ...
                >
                  ...
                  {/* Property Value */}
                  <div className="vb6-property-value">
                    {renderPropertyEditor(propDef, value)}
                  </div>
                </div>
              );
            })}
