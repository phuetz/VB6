...
const VB6_FUNCTIONS = [
  ...
  {
    name: 'Mid',
    description: 'Returns middle characters',
...
const AdvancedCodeEditor: React.FC = () => {
  ...
  const getIntelliSenseItems = useCallback(
    (currentWord: string) => {
      ...
      if (state.selectedControls.length === 1) {
        ...
        controlProps
          .filter(prop => prop.toLowerCase().startsWith(currentWord.toLowerCase()))
          .forEach(prop => {
            items.push({
              label: prop,
              type: 'property',
    ...
    (e: React.KeyboardEvent) => {
      if (intelliSenseVisible) {
        switch (e.key) {
          case 'ArrowDown':
            e.preventDefault();
            setSelectedIntelliSense(prev => (prev < intelliSenseItems.length - 1 ? prev + 1 : 0));
            break;
          case 'ArrowUp':
            e.preventDefault();
            setSelectedIntelliSense(prev => (prev > 0 ? prev - 1 : intelliSenseItems.length - 1));
            break;
          case 'Enter':
          case 'Tab':
            e.preventDefault();
  ...
  const insertIntelliSenseItem = useCallback(
        ...
        <div className="absolute left-0 top-0 bottom-0 w-12 bg-gray-100 border-r border-gray-300 select-none">
          {code.split('\n').map((_, index) => (
            <div
              ...
              style={{
                backgroundColor: errorMarkers.some(e => e.line === index + 1)
                  ? '#ffebee'
                  : 'transparent',
        ...
        <div className="absolute bottom-0 right-0 mb-2 mr-2">
          <div className="bg-red-100 border border-red-400 rounded p-2 max-w-sm">
            ...
            {errorMarkers.length > 3 && (
              <div className="text-xs text-red-600">+{errorMarkers.length - 3} more errors...</div>
            )}
          </div>
        </div>
      )}
