...
import { aiIntelliSenseEngine, CompletionItem, CompletionItemKind, CompletionContext } from '../../services/AIIntelliSenseEngine';
...
export const AIIntelliSenseProvider: React.FC<AIIntelliSenseProviderProps> = ({
  ...
  const convertToMonacoSuggestion = useCallback((
    ...
    if (item.tags?.includes(1)) { // Deprecated
    ...
    if (item.source === 'ai') {
      ...
      label = `📄 ${label}`;
    } else if (item.tags?.includes(3)) { // Popular
      label = `⭐ ${label}`;
    }
  ...
  const provideCompletionItems = useCallback(async (
    ...
    try {
      ...
      setStats(prev => ({
        ...
        averageScore: completions.reduce((sum, c) => sum + c.score, 0) / Math.max(completions.length, 1)
      }));
  ...
  useEffect(() => {
    ...
    const suggestionListener = editor.onDidChangeCursorSelection((e) => {
      ...
      if (model && e.source === 'keyboard' && e.reason === 3) { // Reason 3 = explicit
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              ...
              <div className="bg-gray-50 rounded p-2">
                ...
                <div className="font-semibold text-lg text-green-600">{stats.acceptedSuggestions}</div>
              </div>
...
End If`
  ...
  return examples[functionName] || `' Example usage of ${functionName}
