...
const ComponentsDialog: React.FC = () => {
  ...
  const handleClose = () => {
    windowShowDialog('showComponents', false);
  };
      ...
      >
        ...
        <div className="p-4 flex flex-col h-full">
          <div className="flex gap-2 mb-2">
            <button className="px-2 py-1 bg-gray-300 border border-gray-400 text-xs">
              Controls
            </button>
            <button className="px-2 py-1 bg-gray-100 border border-gray-400 text-xs">
              Designers
            </button>
            <button className="px-2 py-1 bg-gray-100 border border-gray-400 text-xs">
              Insertable Objects
          ...
          <div className="bg-white border border-gray-400 flex-1 overflow-y-auto mb-4">
            {state.components.map((comp: any, index: number) => (
              <div key={index} className="flex items-center p-1 hover:bg-gray-100 text-xs">
                <input type="checkbox" checked={comp.checked} className="mr-2" />
                <span>{comp.name}</span>
              </div>
            ))}
          </div>
