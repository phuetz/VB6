...
export const DBListControl: React.FC<DBListControlProps> = ({
  ...
  const mockDataSources: { [key: string]: DataSource } = {
    ...
    Products: {
      name: 'Products',
      records: [
        { ProductID: 1, ProductName: 'Chai', CategoryID: 1, Price: 18.0 },
        { ProductID: 2, ProductName: 'Chang', CategoryID: 1, Price: 19.0 },
        { ProductID: 3, ProductName: 'Aniseed Syrup', CategoryID: 2, Price: 10.0 },
        { ProductID: 4, ProductName: "Chef Anton's Cajun Seasoning", CategoryID: 2, Price: 22.0 },
      ],
      currentRecord: 0,
  ...
  const loadRowSource = useCallback(() => {
    ...
    if (source) {
      ...
      const items = source.records.map(record => (record[field] ? String(record[field]) : ''));
  ...
  const handleKeyPress = useCallback(
    (event: React.KeyboardEvent) => {
      ...
      const startIndex =
        matchEntry === DBControlConstants.MATCH_EXTENDED && currentIndex >= 0
          ? currentIndex + 1
          : 0;
  ...
  const findItem = useCallback(
    (searchText: string, startIndex: number = 0) => {
...
export const DBComboControl: React.FC<DBComboControlProps> = ({
  ...
  const handleSelectionChange = useCallback(
    (index: number, newText?: string) => {
      const selectedText =
        newText || (index >= 0 && index < currentList.length ? currentList[index] : '');
  ...
  return (
    ...
    >
      ...
      {/* Dropdown list */}
      {(style === DBControlConstants.COMBO_SIMPLE ||
        (style === DBControlConstants.COMBO_DROPDOWN && isDroppedDown)) && (
        <div
          style={{
        ...
        >
          <select
            ...
            size={Math.max(
              1,
              style === DBControlConstants.COMBO_SIMPLE ? Math.floor((height - 21) / 16) : 8
            )}
            style={{
              width: '100%',
              height: '100%',
