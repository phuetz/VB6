...
export const HierarchicalFlexGridControl = forwardRef<HTMLDivElement, HierarchicalFlexGridProps>((props, ref) => {
  const {
    id,
    name,
    left = 0,
  ...
  const vb6Methods = {
    ...
    LoadFromFile: (filename: string) => {
      ...
      console.log('LoadFromFile not implemented in web environment');
    },
  ...
  const renderCell = (row: number, col: number, rowData: FlexGridRow) => {
    ...
    const isSelected = row >= selectedRange.startRow && row <= selectedRange.endRow &&
    ...
    return (
      <div
        ...
        style={{
          ...
          outline: isCurrent && focusRect !== FlexFocusRectConstants.flexFocusNone ? 
                   `${focusRect === FlexFocusRectConstants.flexFocusHeavy ? 2 : 1}px solid #000000` : 'none',
          overflow: 'hidden',
        ...
        >
          {visibleRows.map((rowData, rowIndex) => 
            Array.from({ length: cols }, (_, colIndex) => 
              renderCell(rowIndex, colIndex, rowData)
            )
          )}
        </div>
      </div>
