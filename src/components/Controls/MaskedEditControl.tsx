...
export const MaskedEditControl: React.FC<MaskedEditControlProps> = ({
  ...
  const parseMask = useCallback(
    (maskStr: string): { template: string; positions: number[] } => {
      ...
      while (i < maskStr.length) {
        ...
        if (char === '\\' && i + 1 < maskStr.length) {
          ...
          Object.prototype.hasOwnProperty.call(MASK_CHARS, char) &&
          MASK_CHARS[char as keyof typeof MASK_CHARS]
        ) {
          // Input position
          template += promptChar;
  ...
  const applyMask = useCallback(
    (value: string): { masked: string; valid: boolean } => {
        ...
        if (mask && inputRef.current) {
          ...
          if (hasAllRequiredChars) {
            ...
            const nextElement = document.querySelector(
              `[tabindex="${(parseInt(inputRef.current.tabIndex) || 0) + 1}"]`
            ) as HTMLElement;
            if (nextElement) {
              nextElement.focus();
            }
