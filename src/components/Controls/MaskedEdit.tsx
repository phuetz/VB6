...
const MaskedEdit: React.FC<MaskedEditProps> = ({
  ...
  const extractValue = useCallback(
    (formattedValue: string, maskStr: string) => {
  ...
  const validateChar = useCallback(
    (char: string, position: number, maskStr: string) => {
  ...
  const getNextInputPosition = useCallback(
    (position: number, maskStr: string, direction: 'forward' | 'backward' = 'forward') => {
  ...
  const handleInputChange = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (control.events?.onChange) {
        ...
        control.events.onChange(
          'ClipText',
          promptInclude ? maskedValue : extractValue(maskedValue, mask)
        );
      }
  ...
  const displayValue = passwordChar ? inputValue.replace(/./g, passwordChar) : inputValue;
