...
export const OptimizedDesignerCanvas: React.FC<OptimizedDesignerCanvasProps> = ({
  ...
  }, [isDragging, selectedControls, controls, zoom, enableSnapping, gridSize, dragOffset, throttledRender, onControlMove]);
  ...
  const handleControlSelect = useCallback((controlId: string) => {
    onControlSelect([controlId]);
  }, [onControlSelect]);
  
  // Optimized control update handler
  const handleControlUpdate = useCallback((controlId: string, updates: Partial<Control>) => {
