...
export const PropertyPanel: React.FC<PropertyPanelProps> = ({ control, onPropertyChange }) => {
    ...
    if (typeof value === 'number') {
      return (
        <input
          type="number"
          value={value}
          onChange={e => onPropertyChange(property, parseFloat(e.target.value) || 0)}
  ...
  const properties = Object.keys(control).filter(p => !['id', 'type'].includes(p));

  return (
    <div
      className="bg-white border border-gray-400 rounded p-3 space-y-2 overflow-auto"
      style={{ maxHeight: '100%' }}
    >
      {properties.map(prop => (
