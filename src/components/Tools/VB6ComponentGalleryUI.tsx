...
export const VB6ComponentGalleryUI: React.FC<VB6ComponentGalleryUIProps> = ({
  ...
  const filteredAddIns = addIns.filter(addIn => {
    ...
    if (searchText && !addIn.name.toLowerCase().includes(searchText.toLowerCase()) && 
        !addIn.description.toLowerCase().includes(searchText.toLowerCase())) return false;
    return true;
  });
  ...
  const getStatusColor = (status: VB6ComponentStatus): string => {
    switch (status) {
      case VB6ComponentStatus.INSTALLED: return '#00AA00';
      case VB6ComponentStatus.REGISTERED: return '#0080FF';
      case VB6ComponentStatus.AVAILABLE: return '#666666';
      case VB6ComponentStatus.BROKEN: return '#FF0000';
      case VB6ComponentStatus.NEEDS_UPDATE: return '#FF8800';
      case VB6ComponentStatus.DOWNLOADING: return '#8800FF';
      case VB6ComponentStatus.INSTALLING: return '#0088FF';
  ...
  const getStateColor = (state: VB6AddInState): string => {
    switch (state) {
      case VB6AddInState.LOADED: return '#00AA00';
      case VB6AddInState.UNLOADED: return '#666666';
      case VB6AddInState.DISABLED: return '#888888';
      case VB6AddInState.ERROR: return '#FF0000';
      case VB6AddInState.LOADING: return '#0088FF';
      case VB6AddInState.UNLOADING: return '#FF8800';
      default: return '#666666';
    }
  };
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ flex: 1, overflow: 'hidden', display: 'flex' }}>
            ...
            <>
              ...
              <div style={{
                ...
                <div style={{ marginBottom: '8px' }}>
                  <label style={{ display: 'block', fontSize: '7pt', fontWeight: 'bold' }}>Search:</label>
                  <input
                    type="text"
                    value={searchText}
                    onChange={(e) => setSearchText(e.target.value)}
                    style={{ width: '100%', padding: '2px', border: '1px inset #C0C0C0', fontSize: '8pt' }}
                    placeholder="Search components..."
                  />
                </div>

                <div style={{ marginBottom: '8px' }}>
                  <label style={{ display: 'block', fontSize: '7pt', fontWeight: 'bold' }}>Category:</label>
                  <select
                ...
                <div style={{ marginBottom: '8px' }}>
                  <label style={{ display: 'block', fontSize: '7pt', fontWeight: 'bold' }}>Type:</label>
                  <select
                    value={selectedType}
                ...
                <div style={{ marginBottom: '8px' }}>
                  <label style={{ display: 'block', fontSize: '7pt', fontWeight: 'bold' }}>Status:</label>
                  <select
                ...
                <div style={{ marginBottom: '8px' }}>
                  <label style={{ display: 'block', fontSize: '7pt', fontWeight: 'bold' }}>Min Rating:</label>
                  <input
                    type="range"
                    min="0"
                    max="5"
              ...
              {/* Component List */}
              <div style={{ flex: 1, backgroundColor: '#FFFFFF', border: '1px inset #C0C0C0', overflow: 'auto' }}>
                ...
                {components.map(component => (
                  <div
                    ...
                    style={{
                      ...
                      cursor: 'pointer',
                      backgroundColor: selectedComponent?.id === component.id ? '#E0E0FF' : 'transparent'
                    }}
                    onClick={() => setSelectedComponent(component)}
                  >
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start' }}>
                      <div style={{ flex: 1 }}>
                        <div style={{ fontWeight: 'bold', fontSize: '8pt' }}>{component.name} v{component.version}</div>
                        ...
                        <div style={{ fontSize: '7pt', color: '#888' }}>
                          {component.type.toUpperCase()} • {component.category.replace('_', ' ')} • {formatFileSize(component.fileSize)}
                        </div>
                        <div style={{ fontSize: '7pt', color: '#888' }}>
                          {renderStars(component.rating)} ({component.downloadCount.toLocaleString()} downloads)
                        </div>
                      </div>
                      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'end', gap: '4px' }}>
                        ...
                        {(component.status === VB6ComponentStatus.INSTALLED || component.status === VB6ComponentStatus.REGISTERED) && (
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                ...
                {selectedComponent ? (
                  <div style={{ fontSize: '7pt' }}>
                    <div style={{ marginBottom: '8px' }}>
                      ...
                      <div>Rating: {renderStars(selectedComponent.rating)} ({selectedComponent.rating}/5)</div>
                    </div>
          ...
          {activeTab === 'addins' && (
            <>
              ...
              <div style={{
                ...
                <div style={{ marginBottom: '8px' }}>
                  <label style={{ display: 'block', fontSize: '7pt', fontWeight: 'bold' }}>State:</label>
                  <select
                      ...
                      <div style={{ flex: 1 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '2px' }}>
                          <input
                            ...
                            style={{ margin: 0 }}
                          />
                          <div style={{ fontWeight: 'bold', fontSize: '8pt' }}>{addIn.displayName}</div>
                        </div>
                        <div style={{ fontSize: '7pt', color: '#666', marginBottom: '2px' }}>
                          {addIn.description}
                        </div>
                        ...
                        <div style={{ fontSize: '7pt', color: '#888' }}>
                          Commands: {addIn.providesCommands.length} • Menus: {addIn.providesMenus.length} • Toolbars: {addIn.providesToolbars.length}
                        </div>
                      </div>
                      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'end', gap: '4px' }}>
                        <div style={{ 
                ...
                {selectedAddIn ? (
                  <div style={{ fontSize: '7pt' }}>
                    ...
                    {selectedAddIn.supportsEvents.length > 0 && (
                      <div style={{ marginBottom: '8px' }}>
                        <div style={{ fontWeight: 'bold', marginBottom: '2px' }}>Supported Events:</div>
                    ...
                    {(selectedAddIn.requiredAddIns.length > 0 || selectedAddIn.conflictingAddIns.length > 0) && (
                      <div style={{ marginBottom: '8px' }}>
                        <div style={{ fontWeight: 'bold', marginBottom: '2px' }}>Dependencies:</div>
                        ...
                        {selectedAddIn.conflictingAddIns.map(conf => (
                          <div key={conf} style={{ marginLeft: '8px', fontSize: '6pt', color: '#FF0000' }}>
                            • Conflicts: {conf}
                          </div>
                        ))}
                      </div>
                    ...
                    {selectedAddIn.lastError && (
                      <div style={{ marginBottom: '8px' }}>
                        <div style={{ fontWeight: 'bold', marginBottom: '2px', color: '#FF0000' }}>Last Error:</div>
                        <div style={{ color: '#FF0000', fontSize: '6pt' }}>{selectedAddIn.lastError}</div>
                      </div>
                    )}
                  </div>
                ) : (
            ...
            <div style={{ flex: 1, padding: '8px' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', height: '100%' }}>
                <div style={{ backgroundColor: '#FFFFFF', border: '1px inset #C0C0C0', padding: '8px' }}>
                  <div style={{ fontWeight: 'bold', marginBottom: '8px', backgroundColor: '#C0C0C0', padding: '2px' }}>
                    Installed Components ({VB6ComponentGalleryInstance.getInstalledComponents().length})
                  </div>
                  <div style={{ height: 'calc(100% - 30px)', overflow: 'auto' }}>
                    {VB6ComponentGalleryInstance.getInstalledComponents().map(component => (
                      <div key={component.id} style={{ padding: '4px', borderBottom: '1px solid #E0E0E0' }}>
                ...
                <div style={{ backgroundColor: '#FFFFFF', border: '1px inset #C0C0C0', padding: '8px' }}>
                  ...
                  <div style={{ height: 'calc(100% - 30px)', overflow: 'auto' }}>
                    {VB6AddInManagerInstance.getLoadedAddIns().map(addIn => (
                      <div key={addIn.id} style={{ padding: '4px', borderBottom: '1px solid #E0E0E0' }}>
                        <div style={{ fontWeight: 'bold', fontSize: '8pt' }}>{addIn.displayName}</div>
                        <div style={{ fontSize: '7pt', color: '#666' }}>
          ...
          {activeTab === 'settings' && (
            <div style={{ flex: 1, padding: '8px' }}>
              <div style={{ backgroundColor: '#FFFFFF', border: '1px inset #C0C0C0', padding: '16px' }}>
                <div style={{ fontWeight: 'bold', marginBottom: '16px', fontSize: '10pt' }}>
                  Component Gallery & Add-in Manager Settings
                </div>
                
                <div style={{ marginBottom: '16px' }}>
                  <div style={{ fontWeight: 'bold', marginBottom: '8px' }}>Component Statistics:</div>
                  <div style={{ fontSize: '8pt', marginLeft: '8px' }}>
                    Total Components: {VB6ComponentGalleryInstance.getComponentStats().total}<br/>
                    Installed Components: {VB6ComponentGalleryInstance.getComponentStats().installed}<br/>
                    Total Add-ins: {VB6AddInManagerInstance.getAddInStats().total}<br/>
                    Loaded Add-ins: {VB6AddInManagerInstance.getAddInStats().loaded}<br/>
                    Enabled Add-ins: {VB6AddInManagerInstance.getAddInStats().enabled}
                  </div>
                </div>

                <div style={{ marginBottom: '16px' }}>
                  <div style={{ fontWeight: 'bold', marginBottom: '8px' }}>Component Categories:</div>
                  <div style={{ fontSize: '8pt', marginLeft: '8px' }}>
                    {Array.from(VB6ComponentGalleryInstance.getComponentStats().byCategory.entries()).map(([cat, count]) => (
                      <div key={cat}>{cat.replace('_', ' ')}: {count}</div>
                    ))}
                  </div>
                </div>
                ...
                <div style={{ marginBottom: '16px' }}>
                  ...
                  <div style={{ fontSize: '8pt', marginLeft: '8px' }}>
                    {Array.from(VB6AddInManagerInstance.getAddInStats().byType.entries()).map(([type, count]) => (
        ...
        <div style={{
          ...
          <div>
            {error || installProgress || 'Ready'}
          </div>
          <div>
            Components: {components.length} | Add-ins: {addIns.length} | Loaded: {VB6AddInManagerInstance.getLoadedAddIns().length}
          </div>
        </div>
      </div>
    </div>
