...
export interface IDispatch extends IUnknown {
  ...
  Invoke(dispIdMember: number, riid: string, lcid: number, wFlags: number, pDispParams: any[], pVarResult: any): void;
}
...
export class ActiveXWebAssemblyBridge {
   ...
   */
  private coCreateInstance(clsidPtr: number, pUnkOuterPtr: number, dwClsContext: number, riidPtr: number, ppvPtr: number): number {
    try {
      const clsid = this.readGUID(clsidPtr);
      const riid = this.readGUID(riidPtr);
      
      console.log(`CoCreateInstance: CLSID=${clsid}, IID=${riid}`);
   ...
   * Get IDs of names
   */
  private getIDsOfNames(pDispPtr: number, riidPtr: number, rgszNamesPtr: number, cNames: number, lcid: number, rgDispIdPtr: number): number {
    try {
   ...
   */
  private invoke(pDispPtr: number, dispIdMember: number, riidPtr: number, lcid: number, wFlags: number, pDispParamsPtr: number, pVarResultPtr: number, pExcepInfoPtr: number, puArgErrPtr: number): number {
    try {
      const objectId = this.readPointer(pDispPtr);
      const comObject = this.comObjects.get(objectId);
  ...
  private secureMemoryFree(ptr: number): void {
    ...
    const view = new Uint8Array(this.memory.buffer, ptr, Math.min(64, this.memory.buffer.byteLength - ptr));
    view.fill(0);
  }
  ...
  private variantChangeType(pvargDestPtr: number, pvargSrcPtr: number, wFlags: number, vt: number): number {
  ...
  private readGUID(ptr: number): string {
    ...
    return `{${data1.toString(16).padStart(8, '0')}-${data2.toString(16).padStart(4, '0')}-${data3.toString(16).padStart(4, '0')}-${data4.slice(0, 2).map(b => b.toString(16).padStart(2, '0')).join('')}-${data4.slice(2).map(b => b.toString(16).padStart(2, '0')).join('')}}`.toUpperCase();
  }
...
class DOMClobberingProtection {
  private static readonly GLOBAL_OBJECT_NAMES = [
    ...
    'locationbar', 'statusbar', 'directories', 'personalbar', 'console',
    'alert', 'confirm', 'prompt', 'setTimeout', 'setInterval', 'clearTimeout',
    'clearInterval', 'XMLHttpRequest', 'fetch', 'WebSocket', 'Worker'
  ];
...
class SecureCOMObject {
  ...
  private invokeWithTimeout(func: (...args: any[]) => any, args: any[], timeout: number): Promise<any> {
...
class COMObject {
  ...
  private isSafeMethod(name: string, value: any): boolean {
    ...
    if (typeof value === 'function') {
      ...
      const dangerousPatterns = [
        'eval', 'Function', 'require', 'import', 'document',
        'window', 'global', 'process', 'XMLHttpRequest', 'fetch'
      ];
