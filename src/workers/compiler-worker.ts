...
class ServiceWorkerExploitationProtection {
  ...
  private checkActivityTimeout(): void {
    ...
    if (
      inactivityTime > ServiceWorkerExploitationProtection.ACTIVITY_TIMEOUT_MS &&
  ...
  startOperation(operationId: string): boolean {
    if (
      this.activeOperations.size >= ServiceWorkerExploitationProtection.MAX_CONCURRENT_OPERATIONS
    ) {
...
class VB6Lexer {
  ...
  private readOperator(line: number, column: number): Token {
    ...
    switch (char) {
      case '+':
        return { type: TokenType.PLUS, value: char, line, column };
      case '-':
        return { type: TokenType.MINUS, value: char, line, column };
      case '*':
        return { type: TokenType.MULTIPLY, value: char, line, column };
      case '/':
        return { type: TokenType.DIVIDE, value: char, line, column };
      case '=':
        return { type: TokenType.EQUALS, value: char, line, column };
      case '<':
        return { type: TokenType.LESS_THAN, value: char, line, column };
      case '>':
        return { type: TokenType.GREATER_THAN, value: char, line, column };
      case '(':
        return { type: TokenType.LPAREN, value: char, line, column };
      case ')':
        return { type: TokenType.RPAREN, value: char, line, column };
      case ',':
        return { type: TokenType.COMMA, value: char, line, column };
      case '.':
        return { type: TokenType.DOT, value: char, line, column };
      default:
...
function compileUnit(unit: CompilationUnit, options: OptimizationOptions): CompilationResult {
  ...
  if (unit.source.length > 10 * 1024 * 1024) {
    // 10MB limit
    ...
    switch (type) {
      case 'compile-unit':
        try {
          ...
          if (unit.source.length > 5 * 1024 * 1024) {
            // 5MB limit for individual files
