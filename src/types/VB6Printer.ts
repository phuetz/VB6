...
export class VB6Printer {
  ...
  NewPage(): void {
    ...
    if (typeof window !== 'undefined') {
      console.log(`[VB6 Printer] New page: ${this.Page} on ${this.DeviceName}`);
    }
  }
  ...
  EndDoc(): void {
    ...
    if (typeof window !== 'undefined') {
      console.log(`[VB6 Printer] End document: ${this.Page} pages on ${this.DeviceName}`);
  ...
  KillDoc(): void {
    ...
    if (typeof window !== 'undefined') {
      console.log(`[VB6 Printer] Document cancelled on ${this.DeviceName}`);
    }
  }
  ...
  Print(text: string): void {
    if (typeof window !== 'undefined') {
      console.log(`[VB6 Printer] Print at (${this.CurrentX}, ${this.CurrentY}): ${text}`);
    }
  ...
  Line(x1?: number, y1?: number, x2?: number, y2?: number, color?: string): void {
    ...
    if (typeof window !== 'undefined') {
      console.log(`[VB6 Printer] Line from (${startX}, ${startY}) to (${endX}, ${endY})`);
    }
  ...
  Circle(x: number, y: number, radius: number, color?: string, start?: number, end?: number, aspect?: number): void {
    if (typeof window !== 'undefined') {
      console.log(`[VB6 Printer] Circle at (${x}, ${y}) radius ${radius}`);
    }
  ...
  PSet(x: number, y: number, color?: string): void {
    if (typeof window !== 'undefined') {
      console.log(`[VB6 Printer] PSet at (${x}, ${y}) color ${color || this.ForeColor}`);
    }
