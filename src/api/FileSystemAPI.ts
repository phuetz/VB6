...
class FileSystemAPI {
          ...
          if (obj && typeof obj === 'object') {
            for (const [key, value] of Object.entries(obj)) {
              if (dangerousKeys.includes(key)) {
                throw new Error(
                  `Dangerous key "${key}" found at ${path || 'root'} - potential prototype pollution attack`
                );
              }
  ...
  static RemoveDirectory(pathName: string): boolean {
    try {
      ...
      const children = Array.from(FileSystemAPI.simulatedFileSystem.keys()).filter(key =>
  ...
  static FindFirstFile(fileName: string): { handle: number; findData: WIN32_FIND_DATA | null } {
    try {
      ...
      const matches = Array.from(FileSystemAPI.simulatedFileSystem.keys()).filter(key => {
  ...
  static FindNextFile(handle: number): WIN32_FIND_DATA | null {
    try {
      const searchState = FileSystemAPI.safeJSONParse(
        localStorage.getItem(`VB6_SEARCH_${handle}`) || '{}'
      );
