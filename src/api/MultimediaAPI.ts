...
class MultimediaFingerprintingProtection {
  ...
  private getSessionNoise(key: string): number {
    if (!this.audioContextNoise.has(key)) {
      ...
      let hash = 0;
      const sessionKey =
        key + (sessionStorage.getItem('vb6_multimedia_session') || 'multimedia_default');
...
export class WaveOut extends EventEmitter {
  ...
  waveOutOpen(
    format: WAVEFORMATEX,
    callback?: (msg: number, param1: number, param2: number) => void
  ): number {
    try {
      this._format = format;
      this._isOpen = true;
    ...
    try {
      ...
      switch (params.dwItem) {
        ...
        case 2: // MCI_STATUS_POSITION
          params.dwReturn = this._mediaElement
            ? Math.round(this._mediaElement.currentTime * 1000)
            : 0;
          break;
        case 3: // MCI_STATUS_LENGTH
          params.dwReturn =
            this._mediaElement && isFinite(this._mediaElement.duration)
              ? Math.round(this._mediaElement.duration * 1000)
              : 0;
          break;
...
export class ImageProcessor {
  ...
  private sharpen(imageData: ImageData): ImageData {
    return this.convolution(imageData, [0, -1, 0, -1, 5, -1, 0, -1, 0]);
  }

  private edgeDetection(imageData: ImageData): ImageData {
    return this.convolution(imageData, [-1, -1, -1, -1, 8, -1, -1, -1, -1]);
  }

  private emboss(imageData: ImageData): ImageData {
    return this.convolution(imageData, [-2, -1, 0, -1, 1, 1, 0, 1, 2]);
  }
...
export class MultimediaAPI {
  ...
  static mciSendString(
    ...
    try {
      ...
      if (cmd === 'open') {
        ...
        return device
          ? this.mciSendCommand(device.DeviceId, MCI_COMMAND.MCI_PLAY, 0, {})
          : MMSYSERR.MMSYSERR_INVALHANDLE;
      } else if (cmd === 'stop') {
        const alias = parts[1];
        const device = Array.from(this._mciDevices.values()).find(d => d.Alias === alias);
        return device
          ? this.mciSendCommand(device.DeviceId, MCI_COMMAND.MCI_STOP, 0, {})
          : MMSYSERR.MMSYSERR_INVALHANDLE;
      } else if (cmd === 'close') {
        const alias = parts[1];
        const device = Array.from(this._mciDevices.values()).find(d => d.Alias === alias);
        return device
          ? this.mciSendCommand(device.DeviceId, MCI_COMMAND.MCI_CLOSE, 0, {})
          : MMSYSERR.MMSYSERR_INVALHANDLE;
      }
  ...
  // Get device arrays
  static get WaveOutDevices(): WaveOut[] {
    return this._waveOutDevices;
  }
  static get WaveInDevices(): WaveIn[] {
    return this._waveInDevices;
  }
  static get MCIDevices(): Map<number, MCIDevice> {
    return this._mciDevices;
  }
  static get VideoCaptureDevices(): VideoCapture[] {
    return this._videoCaptureDevices;
  }
}
