...
export interface CryptoKey {
  ...
  flags: number;
  keyData?: CryptoKey | globalThis.CryptoKey;
}
...
export class CryptoAPI extends EventEmitter {
  ...
  public async CryptGenKey(
    ...
    try {
        ...
        try {
          switch (algId) {
            ...
            case CryptoAlgorithm.CALG_AES_256: {
              const keyLength =
                algId === CryptoAlgorithm.CALG_AES_128
                  ? 128
                  : algId === CryptoAlgorithm.CALG_AES_192
                    ? 192
                    : 256;
            ...
            case CryptoAlgorithm.CALG_RSA_SIGN:
              keyData = (await window.crypto.subtle.generateKey(
                ...
                dwFlags & CryptoKeyFlags.CRYPT_EXPORTABLE ? true : false,
                algId === CryptoAlgorithm.CALG_RSA_KEYX
                  ? ['encrypt', 'decrypt']
                  : ['sign', 'verify']
              )) as unknown as globalThis.CryptoKey;
              break;
          }
        } catch (err) {
      ...
      const key: CryptoKey = {
        ...
        algId,
        keySpec:
          algId === CryptoAlgorithm.CALG_RSA_SIGN
            ? CryptoKeySpec.AT_SIGNATURE
            : CryptoKeySpec.AT_KEYEXCHANGE,
        flags: dwFlags,
        keyData,
      };
  ...
  public async CryptHashData(hHash: number, pbData: Uint8Array, dwFlags: number): Promise<boolean> {
    try {
      ...
      if (
        ...
        try {
          ...
          const encrypted = await window.crypto.subtle.encrypt(
            { name: 'AES-GCM', iv },
            key.keyData as globalThis.CryptoKey,
            pbData
          );
  ...
  public CryptBinaryToString(pbBinary: Uint8Array, dwFlags: CryptoStringFormat): string | null {
    try {
      switch (dwFlags) {
  ...
  public CryptStringToBinary(pszString: string, dwFlags: CryptoStringFormat): Uint8Array | null {
    try {
      switch (dwFlags) {
        case CryptoStringFormat.CRYPT_STRING_BASE64: {
          // Use browser's atob
...
export class Crypto {
  ...
  public static Initialize(): boolean {
    const result = this.api.CryptAcquireContext(null, null, CryptoProvider.PROV_RSA_AES, 0);
  ...
  public static async HashData(
    ...
    const hashResult = this.api.CryptCreateHash(this.hProv, algMap[algorithm], 0, 0);
    ...
    const hexResult =
      this.api.CryptBinaryToString(result.data, CryptoStringFormat.CRYPT_STRING_HEX) || '';
  ...
  // Encrypt data
  public static async Encrypt(data: string | Uint8Array, password?: string): Promise<string> {
    ...
    const encResult = await this.api.CryptEncrypt(keyResult.hKey, 0, true, 0, dataBytes);
    ...
    // Convert to base64
    const base64Result =
      this.api.CryptBinaryToString(encResult.data, CryptoStringFormat.CRYPT_STRING_BASE64) || '';
  ...
  public static async Decrypt(encryptedData: string, password?: string): Promise<string> {
    ...
    const decResult = await this.api.CryptDecrypt(keyResult.hKey, 0, true, 0, dataBytes);
  ...
  public static ToBase64(data: string | Uint8Array): string {
    ...
    return this.api.CryptBinaryToString(bytes, CryptoStringFormat.CRYPT_STRING_BASE64) || '';
  }

  // Convert from base64
  public static FromBase64(base64: string): Uint8Array {
    return (
      this.api.CryptStringToBinary(base64, CryptoStringFormat.CRYPT_STRING_BASE64) ||
      new Uint8Array(0)
    );
  }
  ...
  public static ToHex(data: string | Uint8Array): string {
    ...
    return this.api.CryptBinaryToString(bytes, CryptoStringFormat.CRYPT_STRING_HEX) || '';
  }

  // Convert from hex
  public static FromHex(hex: string): Uint8Array {
    return (
      this.api.CryptStringToBinary(hex, CryptoStringFormat.CRYPT_STRING_HEX) || new Uint8Array(0)
    );
  }
