...
describe('Plugin System Tests', () => {
  ...
  describe('Plugin Registration and Discovery', () => {
    ...
    it('should prevent duplicate plugin registration', async () => {
      ...
      await expect(pluginSystem.register(plugin)).rejects.toThrow(
    ...
    it('should detect circular dependencies', async () => {
      ...
      expect(() => pluginSystem.resolveDependencies('plugin1')).toThrow(
  ...
  describe('Plugin Lifecycle', () => {
    ...
    it('should handle plugin errors gracefully', async () => {
      ...
      await expect(pluginSystem.activate('error-plugin')).rejects.toThrow('Activation failed');
  ...
  describe('Plugin API and Extensions', () => {
    ...
    it('should register menu items', async () => {
      ...
      expect(menuItems).toContainEqual(expect.objectContaining({ id: 'plugin.menu' }));
    });
    ...
    it('should register toolbar buttons', async () => {
      ...
      expect(buttons).toContainEqual(expect.objectContaining({ id: 'plugin.button' }));
    });
    ...
    it('should register views', async () => {
      ...
      expect(views).toContainEqual(expect.objectContaining({ id: 'plugin.view' }));
    });
  });
  ...
  describe('Plugin Communication', () => {
    it('should enable inter-plugin communication', async () => {
      ...
      expect(receivedMessages).toContainEqual(expect.objectContaining({ data: 'Hello' }));
    });
    ...
    it('should enforce plugin permissions', async () => {
      const plugin: PluginTypes.Plugin = {
        ...
        activate: api => {
          ...
          // Should be denied
          expect(() => api.filesystem.writeFile('/test.txt', 'data')).toThrow(
  ...
  describe('Plugin Updates', () => {
    ...
    it('should rollback failed updates', async () => {
      ...
      await expect(pluginSystem.updatePlugin('rollback-plugin', '1.1.0')).rejects.toThrow(
        'Update failed'
      );
  ...
  describe('Plugin Security', () => {
    ...
    it('should validate plugin signatures', async () => {
      ...
      await expect(pluginSystem.register(plugin)).rejects.toThrow('Invalid plugin signature');
    });
    ...
    it('should scan plugins for malicious code', async () => {
      ...
      await expect(pluginSystem.register(maliciousPlugin)).rejects.toThrow(
        'Security scan failed: Dangerous code patterns detected'
      );
    });
  });
  ...
  describe('Plugin Development Tools', () => {
    ...
    it('should scaffold new plugin', async () => {
      ...
      expect(scaffolded.files).toContainEqual(expect.objectContaining({ name: 'index.js' }));
    });
