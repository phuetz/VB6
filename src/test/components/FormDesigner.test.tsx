...
describe('Form Designer - Selection Management', () => {
  ...
  it('should handle selection rectangle calculation', () => {
    ...
    expect(bounds.right).toBe(Math.max(
      controls[0].left + controls[0].width,
      controls[1].left + controls[1].width
    ));
    expect(bounds.bottom).toBe(Math.max(
      controls[0].top + controls[0].height,
      controls[1].top + controls[1].height
    ));
  });
});
...
function setZOrder(control: VB6Control, direction: 'front' | 'back', allControls: VB6Control[]): VB6Control {
...
function alignControls(controls: VB6Control[], alignment: 'left' | 'right' | 'top' | 'bottom'): VB6Control[] {
...
function distributeControls(controls: VB6Control[], direction: 'horizontal' | 'vertical'): VB6Control[] {
  ...
  return sorted.map((control, index) => {
    ...
    const position = direction === 'horizontal'
      ? first.left + (spacing * index)
      : first.top + (spacing * index);
...
function makeSameSize(controls: VB6Control[], dimension: 'width' | 'height' | 'both'): VB6Control[] {
  if (controls.length < 2) return controls;
...
function centerOnForm(control: VB6Control, formSize: { width: number; height: number }): VB6Control {
  return {
    ...control,
...
function getRubberBandSelection(controls: VB6Control[], rubberBand: {
  startX: number;
  startY: number;
  endX: number;
  endY: number;
...
function pasteFromClipboard(clipboard: VB6Control[], offsetX: number, offsetY: number): VB6Control[] {
