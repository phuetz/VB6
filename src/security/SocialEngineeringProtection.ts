...
export interface DarkPattern {
  name: string;
  type: 'bait_switch' | 'roach_motel' | 'forced_continuity' | 'hidden_costs' | 'confirmshaming' | 'trick_questions' | 'sneak_basket' | 'misdirection' | 'price_manipulation' | 'false_urgency';
  element: string;
...
export interface ManipulationAttempt {
  type: string;
  targetEmotion: 'fear' | 'greed' | 'urgency' | 'trust' | 'authority' | 'social_proof' | 'reciprocity' | 'commitment' | 'scarcity';
...
export class SocialEngineeringProtection {
  ...
  private readonly MANIPULATION_KEYWORDS = {
    urgency: ['urgent', 'expires', 'limited time', 'act now', 'hurry', 'last chance', 'ending soon', 'only today'],
    fear: ['warning', 'alert', 'danger', 'risk', 'threat', 'virus', 'hacked', 'compromised', 'suspended'],
    greed: ['free', 'win', 'prize', 'reward', 'bonus', 'profit', 'earn', 'money', 'cash'],
    authority: ['official', 'verified', 'certified', 'approved', 'authorized', 'government', 'legal'],
    ...
    social: ['others', 'people', 'users', 'customers', 'trending', 'popular', 'everyone', 'million']
  };
  ...
  private initializeProtection(): void {
    ...
    console.log('SocialEngineeringProtection initialized with config:', this.config);
  }
  ...
  private checkDeceptiveStyling(element: HTMLElement): void {
    ...
    if (opacity < 0.3 || fontSize < 8) {
      ...
      const importantKeywords = ['unsubscribe', 'cancel', 'terms', 'privacy', 'decline', 'no thanks'];
  ...
  private checkFakeUrgency(element: Element): void {
    ...
    timerElements.forEach((timer) => {
      ...
      setTimeout(() => {
        ...
        if (currentValue === previousValue || (previousValue && currentValue && currentValue > previousValue)) {
  ...
  private scanForManipulativeContent(): void {
    ...
    textElements.forEach((element) => {
      ...
      for (const [emotion, keywords] of Object.entries(this.MANIPULATION_KEYWORDS)) {
        ...
        if (keywordCount >= 2) { // Multiple keywords indicate manipulation
          this.recordManipulationAttempt({
  ...
  private updateEmotionalState(profile: UserBehaviorProfile): void {
    ...
    if (profile.vulnerabilityScore > 0.8) {
      this.recordThreat({
        ...
        blocked: false,
        evidence: [`vulnerability: ${profile.vulnerabilityScore}`, `state: ${profile.emotionalState}`],
        timestamp: Date.now()
      });
    }
  }
  ...
  private monitorEmotionalTriggers(): void {
    ...
    const fearElements = document.querySelectorAll('.warning, .alert, .danger, [class*="virus"], [class*="threat"]');
    ...
    const scarcityElements = document.querySelectorAll('[class*="limited"], [class*="remaining"], [class*="stock"]');
    ...
    const protectButton = (button: HTMLElement) => {
      ...
      button.onclick = (event) => {
        ...
        if (profile.vulnerabilityScore > 0.7) {
          ...
          if (confirm('You seem stressed. Are you sure you want to proceed? Take a moment to think.')) {
            // Add 3 second delay
  ...
  private getSessionId(): string {
    return sessionStorage.getItem('se_session_id') || 
      (() => {
  ...
  private alertUser(threat: PsychologicalThreat): void {
    if (this.config.userProtectionLevel === 'maximum') {
      alert(`Security Warning: ${threat.description}\n\nThis page may be trying to manipulate you.`);
    }
  }
  ...
  getSecurityStats(): {
    ...
    const emotionCounts = this.manipulationAttempts.reduce((acc, attempt) => {
      acc[attempt.targetEmotion] = (acc[attempt.targetEmotion] || 0) + 1;
      return acc;
    }, {} as Record<string, number>);
    
    const mostTargeted = Object.entries(emotionCounts)
      .sort((a, b) => b[1] - a[1])[0]?.[0] || 'none';
    
    const avgVulnerability = Array.from(this.userProfiles.values())
      .reduce((sum, profile) => sum + profile.vulnerabilityScore, 0) / 
  ...
  shutdown(): void {
    ...
    console.log('SocialEngineeringProtection shutdown complete');
  }
}
