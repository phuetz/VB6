...
export class PhysicalSecurityProtection {
   ...
   */
  private analyzeUltrasonicPatterns(
    bins: Array<{ bin: number; amplitude: number; frequency: number }>
  ): {
    suspicious: boolean;
    frequency: number;
    amplitude: number;
    ...
    const dominant = bins.reduce((max, bin) => (bin.amplitude > max.amplitude ? bin : max));

    // Check against known air-gap techniques
    if (
      dominant.frequency >= this.AIR_GAP_TECHNIQUES.acoustic.speakear.minFreq &&
  ...
  private analyzeKeystrokeTimings(): void {
    const recentSignals = this.acousticSignals.filter(
      s =>
        Date.now() - s.timestamp < 10000 && // Last 10 seconds
        s.frequency > 3000 &&
        s.frequency < 15000 // Keyboard range
    );
    ...
    if (recentSignals.length > 20) {
      ...
      if (stdDev < 50) {
        ...
        this.recordThreat({
          ...
          medium: 'acoustic',
          evidence: [
            `keystroke_count: ${recentSignals.length}`,
            `timing_stddev: ${stdDev.toFixed(2)}`,
          ],
          mitigated: false,
          timestamp: Date.now(),
        });
        ...
        if (mutation.type === 'attributes' && mutation.attributeName === 'style') {
          ...
          if (filter && filter.includes('brightness')) {
            ...
            if (match) {
              ...
              brightnessChanges = brightnessChanges.filter(c => Date.now() - c.time < 5000);
  ...
  private analyzeLEDPatterns(): void {
    const recentSignals = this.opticalSignals.filter(
      s => s.type === 'led' && Date.now() - s.timestamp < 5000
    );
  ...
  private monitorTEMPESTIndicators(): void {
    ...
    setInterval(() => {
      ...
      if (currentHash !== lastScreenHash) {
        ...
        const emSignals = this.emSignals.filter(s => Date.now() - s.timestamp < 1000);
    ...
    if (recentAcoustic.length > 0 && recentOptical.length > 0) {
      ...
      for (const acoustic of recentAcoustic) {
        for (const optical of recentOptical) {
          if (Math.abs(acoustic.timestamp - optical.timestamp) < 100) {
            // 100ms window
            this.recordThreat({
