...
export class APTProtection {
  ...
  private monitorStorageAccess(): void {
    ...
    ['setItem', 'getItem', 'removeItem'].forEach(method => {
      ...
      (localStorage as unknown as Record<string, unknown>)[method] = function (...args: unknown[]) {
        const key = args[0];
  ...
  private initializeNetworkMonitoring(): void {
    ...
    if (typeof XMLHttpRequest !== 'undefined') {
      ...
      (window as unknown as Record<string, unknown>).XMLHttpRequest = class extends OriginalXHR {
        open(
          method: string,
          url: string,
          async?: boolean,
  ...
  private analyzeNetworkRequest(url: string, options?: RequestInit): void {
    try {
      ...
      const suspicious = this.SUSPICIOUS_DOMAINS.some(pattern => pattern.test(urlObj.hostname));
  ...
  private analyzeWebSocketConnection(url: string): void {
    try {
      ...
      const suspicious =
        this.SUSPICIOUS_DOMAINS.some(pattern => pattern.test(urlObj.hostname)) ||
  ...
  private analyzeScriptInjection(script: HTMLScriptElement): void {
    ...
    if (src) {
      try {
        ...
        const suspicious = this.SUSPICIOUS_DOMAINS.some(pattern => pattern.test(url.hostname));
   ...
   */
  private recordUserAction(type: string, eventData: unknown): void {
   ...
   */
  private calculateSuspiciousScore(type: string, eventData: unknown): number {
    let score = 0;
    ...
    if (type === 'visibility_change' && typeof eventData === 'object' && eventData !== null && 'hidden' in eventData && (eventData as { hidden: boolean }).hidden) {
      score += 0.2; // Activity when tab is hidden is suspicious
    }

    if (type === 'keydown' && typeof eventData === 'object' && eventData !== null) {
      const keyEvent = eventData as { ctrlKey?: boolean; shiftKey?: boolean };
  ...
  private checkForEnvironmentChanges(profile: BehavioralProfile): void {
    ...
    const metadata = profile.metadata as { timezone?: string } | undefined;
    if (!metadata?.timezone) {
      profile.metadata = { ...metadata, timezone: currentTimezone };
    } else if (metadata.timezone !== currentTimezone) {
      this.recordThreat({
  ...
  private cleanupOldData(): void {
    ...
    this.networkConnections = this.networkConnections.filter(conn => conn.timestamp > cutoffTime);
  }
  ...
  getSecurityStats(): {
    ...
    const criticalThreats = this.threats.filter(t => t.severity === 'high' && !t.mitigated).length;
    ...
    const anomalyScores = Array.from(this.behavioralProfiles.values()).map(p => p.anomalyScore);
