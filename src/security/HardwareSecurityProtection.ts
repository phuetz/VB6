// Experimental Web API type declarations (not in standard lib.dom)
...
interface WebUSBApi extends EventTarget {
  addEventListener(type: string, callback: (event: USBDeviceEvent) => void): void;
  requestDevice(options: Record<string, unknown>): Promise<USBDeviceRecord>;
}
...
interface WebHIDApi extends EventTarget {
  addEventListener(type: string, callback: (event: { device: WebHIDDevice }) => void): void;
}
...
export class HardwareSecurityProtection {
  ...
  private initializeUSBProtection(): void {
    ...
    if (nav.usb) {
      ...
      usb.addEventListener('connect', (event: USBDeviceEvent) => {
        this.handleUSBConnection(event.device);
      });

      usb.addEventListener('disconnect', (event: USBDeviceEvent) => {
        this.handleUSBDisconnection(event.device);
      });
    }

    // Monitor WebUSB API usage
    if (nav.usb && nav.usb.requestDevice) {
      const usb = nav.usb;
      const originalRequestDevice = usb.requestDevice.bind(usb);

      usb.requestDevice = async (options: Record<string, unknown>) => {
   ...
   */
  private handleUSBConnection(device: USBDeviceRecord): void {
    ...
    const recentConnections = this.usbEvents.filter(
      e =>
        e.type === 'connect' &&
    ...
    if (recentConnections.length >= this.SUSPICIOUS_USB_PATTERNS.rapidConnect.threshold) {
      this.recordThreat({
        ...
        timestamp: Date.now(),
        evidence: [
          `${recentConnections.length} connections in ${this.SUSPICIOUS_USB_PATTERNS.rapidConnect.window}ms`,
        ],
      });
    }
   ...
   */
  private checkMaliciousDevice(
    device: USBDeviceRecord
  ): { vid: string; pid: string; name: string } | undefined {
    if (!device.vendorId || !device.productId) return undefined;

    const vid = device.vendorId.toString(16).padStart(4, '0');
    const pid = device.productId.toString(16).padStart(4, '0');

    return this.MALICIOUS_USB_DEVICES.find(
      malicious => malicious.vid === vid && malicious.pid === pid
    );
  }
   ...
   */
  private generateDeviceFingerprint(device: USBDeviceRecord): string {
    const components = [
      device.vendorId,
      device.productId,
  ...
  private monitorHIDDevices(): void {
    ...
    hid.addEventListener('connect', (event: { device: WebHIDDevice }) => {
   ...
   */
  private isKeyboardLikeDevice(device: WebHIDDevice): boolean {
      ...
      if (contextType === 'webgl' || contextType === 'webgl2') {
        ...
        const context = (
          originalGetContext as unknown as {
            apply: (thisArg: HTMLCanvasElement, args: unknown[]) => RenderingContext | null;
          }
        ).apply(this, [contextType, ...args]);

        if (context) {
          protection.wrapWebGLContext(context as WebGLRenderingContext | WebGL2RenderingContext);
        }
  ...
  private wrapWebGLContext(gl: WebGLRenderingContext | WebGL2RenderingContext): void {
    ...
    (gl as unknown as Record<string, (...args: unknown[]) => unknown>).bufferData = function (
      target: number,
   ...
   */
  private generateDeviceId(device: USBDeviceRecord): string {
   ...
   */
  private handleUSBDisconnection(device: USBDeviceRecord): void {
    const deviceId = this.generateDeviceId(device);
   ...
   */
  private isSuspiciousUSBRequest(options: Record<string, unknown>): boolean {
    const filters = options.filters as Array<{ vendorId?: number; productId?: number }> | undefined;
    ...
    for (const filter of filters) {
      const match = this.MALICIOUS_USB_DEVICES.find(
        device =>
  ...
  private monitorSerialDevices(): void {
    ...
    serial.requestPort = async (options?: Record<string, unknown>) => {
      this.recordThreat({
  ...
  private monitorUSBDataRates(): void {
    ...
    const recentTransfers = this.usbEvents.filter(
      e => e.type === 'data_transfer' && Date.now() - e.timestamp < 60000 // Last minute
    );
  ...
  private checkSideChannelIndicators(): void {
    ...
    const variance =
      timingData.reduce((sum, t) => sum + Math.pow(t - avgTiming, 2), 0) / timingData.length;
  ...
  getSecurityStats(): {
    ...
    const suspiciousDevices = Array.from(this.knownDevices.values()).filter(
      d => d.suspiciousActivity > 0
    ).length;

    const firmwareIssues = this.threats.filter(
      t => t.type.includes('firmware') || t.type.includes('rootkit')
    ).length;
