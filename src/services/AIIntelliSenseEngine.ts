...
export class AIIntelliSenseEngine {
  ...
  private parseContext(context: CompletionContext): ParsedIntelliSenseContext {
    ...
    const endOffset = Math.min(
      code.length,
      this.getOffset(code, position) + this.config.contextWindow
    );
  ...
  private getKeywordSuggestions(
    parseResult: ParsedIntelliSenseContext,
    ...
    const applicableKeywords = this.VB6_KEYWORDS.filter(keyword => {
      ...
      if (
        ...
        !['Dim', 'Private', 'Public', 'Const'].includes(keyword)
      ) {
        return false;
      }
  ...
  private getFunctionSuggestions(
    ...
    this.VB6_FUNCTIONS.filter(func =>
  ...
  private generateEventHandlerSuggestions(
    parseResult: any,
    context: CompletionContext
  ): CompletionItem[] {
    const suggestions: CompletionItem[] = [];
  ...
  private generatePropertyValueSuggestions(
    parseResult: any,
    context: CompletionContext
  ): CompletionItem[] {
    const suggestions: CompletionItem[] = [];
