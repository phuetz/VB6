...
const VB6_KEYWORDS = [
  ...
  'Const', 'Currency', 'Debug', 'Dim', 'Do', 'Double', 'Each', 'Else', 'ElseIf',
  'Empty', 'End', 'Enum', 'Erase', 'Error', 'Event', 'Exit', 'False', 'For',
  'Friend', 'Function', 'Get', 'GoSub', 'Goto', 'If', 'Implements', 'In',
  'Integer', 'Is', 'Let', 'Like', 'Long', 'Loop', 'Me', 'Mod', 'New', 'Next',
  'Not', 'Nothing', 'Null', 'On', 'Option', 'Optional', 'Or', 'ParamArray',
  'Preserve', 'Private', 'Property', 'Public', 'RaiseEvent', 'ReDim', 'Rem',
  'Resume', 'Return', 'Select', 'Set', 'Single', 'Static', 'Step', 'Stop',
  'String', 'Sub', 'Then', 'To', 'True', 'Type', 'TypeOf', 'Until', 'Variant',
  'Wend', 'While', 'With', 'Xor'
];

// Block start keywords
const BLOCK_START_KEYWORDS = [
  'Sub', 'Function', 'Property', 'If', 'Select', 'For', 'Do', 'While', 'With', 'Type', 'Enum'
];

// Block end keywords
const BLOCK_END_KEYWORDS = [
  'End Sub', 'End Function', 'End Property', 'End If', 'End Select', 'Next', 'Loop', 'Wend', 'End With', 'End Type', 'End Enum'
];
...
export class VB6CodeFormatter extends EventEmitter {
  ...
  private getLineType(line: string): LineType {
    ...
    if (lower.startsWith('dim ') || lower.startsWith('private ') || 
        lower.startsWith('public ') || lower.startsWith('const ') ||
        lower.startsWith('static ') || lower.startsWith('global ')) {
      return LineType.Declaration;
    }
    
    if (lower.startsWith('sub ') || lower.startsWith('function ') || 
        lower.startsWith('property ')) {
      return LineType.ProcedureStart;
    }
    
    if (lower.startsWith('end sub') || lower.startsWith('end function') || 
        lower.startsWith('end property')) {
      return LineType.ProcedureEnd;
    }
    
    if (lower.startsWith('if ') || lower.startsWith('select ') || 
        lower.startsWith('for ') || lower.startsWith('do ') ||
        lower.startsWith('while ') || lower.startsWith('with ')) {
  ...
  private isBlockStart(line: string): boolean {
    const lower = line.toLowerCase();
    return BLOCK_START_KEYWORDS.some(keyword => 
      lower.startsWith(keyword.toLowerCase() + ' ') || 
  ...
  private isBlockEnd(line: string): boolean {
    const lower = line.toLowerCase();
    return BLOCK_END_KEYWORDS.some(keyword => 
      lower.startsWith(keyword.toLowerCase()) || 
      lower === keyword.toLowerCase()
    );
  }
  ...
  private alignGroup(group: CodeLine[], pattern: RegExp): CodeLine[] {
    ...
    const maxLengths = matches.reduce((acc, match) => {
      if (!match) return acc;
      return match.slice(1).map((part, i) => 
        Math.max(acc[i] || 0, part.trim().length)
      );
    }, [] as number[]);
    ...
    return group.map((line, i) => {
      ...
      const aligned = match.slice(1).map((part, j) => {
  ...
  private groupByType(lines: CodeLine[]): CodeLine[] {
    ...
    lines.forEach(line => {
      if (line.text.toLowerCase().startsWith('option ')) {
      ...
      } else if (line.text.toLowerCase().startsWith('type ') || 
        ...
        currentGroup.push(line);
        if (line.type === LineType.ProcedureEnd || 
  ...
  public getFormattingDiagnostics(code: string): Array<{ line: number; message: string; severity: 'info' | 'warning' | 'error' }> {
