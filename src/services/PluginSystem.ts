...
export class PluginSystem {
      ...
      storage: {
        ...
        set: (key, value) => {
          ...
          try {
            ...
            if (serializedValue.length > 1024 * 1024) { // 1MB limit per item
        ...
        clear: () => {
          ...
          try {
            ...
            console.log(`Cleared ${keysToRemove.length} plugin storage entries`);
  ...
  async installPlugin(pluginUrl: string): Promise<Plugin> {
    ...
    try {
      ...
      console.log(`Starting installation of plugin from ${pluginUrl}`);
      ...
      if (this.plugins.has(manifest.id)) {
        ...
        console.log(`Updating plugin ${manifest.id} from ${existing.version} to ${manifest.version}`);
      }
      ...
      console.log(`Successfully installed plugin ${plugin.id} version ${plugin.version}`);
  ...
  private createPluginContext(plugin: Plugin): PluginContext {
    return {
      ...
      globalState: {
        ...
        update: (key: string, value: StorageValue) => this.api.storage.set(`${plugin.id}:${key}`, value),
      },
      workspaceState: {
  ...
  public dispose(): void {
    ...
    this.sandboxes.forEach((sandbox, pluginId) => {
      try {
        sandbox.terminate();
        console.log(`Terminated sandbox for plugin: ${pluginId}`);
      } catch (error) {
    ...
    console.log('PluginSystem disposed');
  }
}
