...
export class VB6CollaborationEngine extends EventEmitter {
  ...
  private applyOperationLocally(documentId: string, operation: CRDTOperation): void {
    ...
    document.vectorClock = this.mergeVectorClocks(
      document.vectorClock,
      transformedOperation.vectorClock
    );
    document.version++;
  ...
  private transformOperation(operation: CRDTOperation, document: DocumentState): CRDTOperation {
    ...
    const concurrentOps = document.operations.filter(
      op =>
        this.isConcurrent(op.vectorClock, operation.vectorClock) && op.author !== operation.author
    );
