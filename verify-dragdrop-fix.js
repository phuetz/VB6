...
function checkDragDropProvider() {
  ...
  const problematicPatterns = [
    {
      pattern: /useEffect\([^}]+\}, \[[^\]]*set[A-Z][^,\]]*[,\]]/,
      description: 'Setter function in useEffect dependencies',
    },
    {
      pattern: /useEffect\([^}]+\}, \[[^\]]*addLog[,\]]/,
      description: 'addLog in useEffect dependencies',
    },
    {
      pattern: /useCallback\([^}]+\}, \[[^\]]*addLog[,\]]/,
      description: 'addLog in useCallback dependencies causing potential loops',
    },
  ];
  ...
  const fixes = [
    ...
    {
      pattern:
        /}, \[dropZones\.length\]\); \/\/ INFINITE LOOP FIX: Remove addLog from dependencies/,
      description: 'Fixed initialization useEffect dependencies',
    },
    {
      pattern:
        /\[activeDropZone, dropZones, dragData, vibrate, playSound\] \/\/ INFINITE LOOP FIX: Remove addLog from dependencies/,
