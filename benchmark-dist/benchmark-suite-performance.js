...
`);
    ...
    printOverallAnalysis(results) {
        ...
        testCases.forEach(testName => {
            const result = results.get(testName);
            const avgThroughput = componentNames.reduce((sum, compName) => sum + result[compName].throughput, 0) / componentNames.length;
            console.log(`  ${testName}: ${avgThroughput.toFixed(0)} lines/sec average`);
        });
        console.log('\n💾 Memory Usage Analysis:');
        testCases.forEach(testName => {
            const result = results.get(testName);
            const totalMemory = componentNames.reduce((sum, compName) => sum + result[compName].memoryUsage, 0);
