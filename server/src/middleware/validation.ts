...
export const validateADOConnection = [
  ...
  body('mode')
    .optional()
    .isInt({ min: 0, max: 16 })
    .withMessage('Invalid connection mode'),
  body('cursorLocation')
    .optional()
    .isIn([1, 2, 3])
    .withMessage('Invalid cursor location'),
  handleValidationErrors,
];
...
export const validateDAODatabase = [
  ...
  body('exclusive')
    .optional()
    .isBoolean()
    .withMessage('Exclusive must be a boolean'),
  body('readOnly')
    .optional()
    .isBoolean()
...
export const validateRDOConnection = [
  body('environmentId')
    .optional()
    .isString()
    .withMessage('Environment ID must be a string'),
  body('dsn')
    .optional()
    .isString()
    .withMessage('DSN must be a string'),
  body('connect')
    .optional()
    .isString()
    .withMessage('Connect string must be a string'),
  body('prompt')
    .optional()
    .isIn([1, 2, 3, 4])
...
export const validateReportOpen = [
  ...
  body('reportKind')
    .optional()
    .isIn([0, 1])
    .withMessage('Invalid report kind'),
  handleValidationErrors,
];
...
export const validateBatchOperation = [
  ...
  body('operations.*.query')
    .isString()
    .notEmpty()
    .withMessage('Each operation must have a query'),
